...
% Find the slice (Z-axis) with the maximum tumor area in the Ground Truth
tumor_pixels_per_slice = squeeze(sum(sum(volume_gt > 0, 1), 2));
[max_pixels, best_slice_idx] = max(tumor_pixels_per_slice);

disp(['Dynamic Selection: Maximum tumor area found at slice Z = ', num2str(best_slice_idx)]);
...
disp('--------------------------------------------------');
disp(['Pcd (Sensitivity):                ', num2str(Sens, '%.4f')]);
